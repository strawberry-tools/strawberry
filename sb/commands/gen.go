	...
	in the "man" directory under the current directory.`,
			...
			run: func(ctx context.Context, cd *simplecobra.Commandeer, r *rootCommand, args []string) error {
				header := &doc.GenManHeader{
					...
					Source:  fmt.Sprintf("Strawberry v%s", hugo.CurrentVersion),
				}
				...
				r.Println("Generating Strawberry man pages in", genmandir, "...")
...
`
	...
	of Strawberry's command-line interface for https://gohugo.io/.

	It creates one Markdown file per command with front matter suitable
	for rendering in Strawberry.`,
			run: func(ctx context.Context, cd *simplecobra.Commandeer, r *rootCommand, args []string) error {
				...
				r.Println("Generating Strawberry command-line documentation in", gendocdir, "...")
	...
	newDocsHelper := func() simplecobra.Commander {
		return &simpleCommand{
			name:  "docshelper",
			short: "Generate some data files for the Strawberry docs.",
